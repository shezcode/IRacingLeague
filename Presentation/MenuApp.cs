...
public class MenuApp
{
    ...
    // The CLI has no auth; a single "active driver" is the logged-in identity that
    // unlocks the private zone and owns the leagues created while it is selected.
    ...
    private User? CreateDriver()
    {
        ...
        string password = AskOptional("Password (blank ok in CLI):", string.Empty);
        ...
        while (true)
        {
            ...
            var choice = AnsiConsole.Prompt(
                new SelectionPrompt<string>()
                    ...
                    .HighlightStyle(new Style(foreground: Color.Green))
                    .AddChoices("View races", "View standings", "View members",
    ...
    private void ScheduleRaceFor(League league)
    {
        ...
        int round = AskInt("Round:");

        var race = _raceService.Create(league.LeagueId, track, car, scheduledAt, lapCount, ambientTemp, round);
    ...
    private void EnterResultFor(League league)
    {
        ...
        try
        {
            var registration = _registrationService.GetById(registrationId);
            var race = _raceService.GetById(raceId);
    ...
    {
        var standings = _standingsService.GetStandings(leagueId).ToList();
        ...
        table.AddColumns("Pos", "Driver", "Points");
        int pos = 1;
        foreach (var r in standings)
            table.AddRow((pos++).ToString(), Markup.Escape(DriverName(r.UserId)), r.Points.ToString());
        AnsiConsole.Write(table);
    }
